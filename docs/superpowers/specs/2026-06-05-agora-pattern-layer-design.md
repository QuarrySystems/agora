...
The engine gains one scoped gate-aware predicate, pulled by the first live gated run (`examples/dogfood-gated`, run 3). In `engine/dep-resolver.ts`, `computeNewlyReady` and `computeSkipped` now treat a dependency as **failed-like** (blocks readiness; cascades skip) when the item is `done` AND `verify?.passed === false` AND declares `inputs.gate.onRed === 'spawn-fix'`. This change was necessary because the original spec omitted gating semantics for the done-but-red case: without this predicate, a red gate item satisfied readiness (its status is done), so downstream items fired against unreviewed work in the same tick; the pattern phase ran after, so the respawned skipped descendants were unreachable, making findings-by-provenance and the downstream skip-and-remap mutually exclusive. The offline demo masked this by using a *failed* gate, which loses the `outputRefs.findings` sentinel entirely. **Scope:** the predicate applies only to items declaring `inputs.gate.onRed === 'spawn-fix'` — red verify on every other item remains report-only, unchanged. The global "verify is report-only" contract holds everywhere outside of declared gates. **Data-edge exemption (landed with the predicate):** the rule is evaluated per (dependent, dependency) pair — a red gate blocks *control-flow* dependents but NOT data consumers of the gate's own outputs: a dependent declaring some `needs[*]` binding with `from === <gate id>` and `select.kind === 'output'` is exempt. This is what lets the gate's own spawned fix item (auto-bound `needs.findings`) ready and consume the findings the red verdict produced. This amendment **supersedes the §11 bullet above** ("Gate semantics for done-but-red … deliberately do NOT hold back downstream") for items that declare `inputs.gate.onRed === 'spawn-fix'`; that bullet remains accurate for every other item. See [dogfood run-3 design](./2026-06-06-dogfood-run3-gated-circleback-design.md) §7 (including its "Data-edge exemption" paragraph) for the driving design and full motivation.

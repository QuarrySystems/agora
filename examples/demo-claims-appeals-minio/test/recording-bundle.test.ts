...
// ---------------------------------------------------------------------------
function makeCleanBundle(): AuditBundle {
  ...
  // Anchor root that matches the Merkle root of the single entry.
  ...
  // Fake anchor that returns the pre-computed root.
  ...
  return {
    ...
    // Attach the fakeAnchor as a non-enumerable symbol so tests can access it
    // without it becoming part of the serialised bundle shape.
    // Actually we'll return both separately.
  } as AuditBundle;
}

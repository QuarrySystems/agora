...
write an `pangolin.config.mjs`, wire up the CLI, register a capability, a subagent,
...
3. **Create your deploy dir.**
   ...
   import {
     ...
     StdoutResultSink,
   } from 'file:///ABS/PATH/TO/pangolin-scale/packages/pangolin-client/dist/index.js';
   import { LocalStorageProvider } from 'file:///ABS/PATH/TO/pangolin-scale/packages/pangolin-storage-local/dist/index.js';
   import { LocalDockerProvider } from 'file:///ABS/PATH/TO/pangolin-scale/packages/pangolin-providers-local-docker/dist/index.js';
...
5. **Wire up the CLI.**
   ...
   ```bash
   alias pangolin='node /ABS/PATH/TO/pangolin-scale/packages/pangolin-cli/dist/index.js'
   ```
...
```json
{
  "mcpServers": {
    "pangolin-scale": {
      "command": "node",
      "args": ["/ABS/PATH/TO/pangolin-scale/packages/pangolin-mcp/dist/bin.js"],
...
- [Architecture overview](/pangolin-scale/explanation/architecture-overview/) — the
  design behind the provider seams you just wired, if you want to read the
  model before going further.
- [Your first offload run](/pangolin-scale/tutorials/first-offload-run/) — graduate from

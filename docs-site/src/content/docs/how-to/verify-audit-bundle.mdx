...
| `checks` | Collect-all per-check results: `chain`, `root`, `signature`, `anchor`, each `{ ok: true \| false \| 'n/a'; detail? }`. Every check is evaluated (no early return), so a failing bundle reports the state of all four. This is what `agora verify` renders as a checklist. |
...
## 4. Hand the bundle to a third party — `agora verify`
...
An independent party re-verifies it with the top-level `agora verify` command:

```sh
agora verify bundle.json
```
...
```text
  agora verify  ·  run_a3f9c2                  ✓ TAMPER-EVIDENT
  ──────────────────────────────────────────────────────────
  ✓ chain        10 entries, hash-linked, no gaps
  ✓ root         merkle c4f1a9…  =  anchored root
  ✓ signature    ed25519 / agora-prod  valid
...
`agora verify` is **not** a pure function of the bundle file. Like `verify()`
...
`agora.config` (the same `orch` context `orch audit` uses):
...
library entry point, exported from `@quarry-systems/agora-orchestrator` so an

...
For the design rationale, see [ADR-0008](/pangolin-scale/explanation/decisions/0008-needs-input-request-stop-restart/) (why request-stop-restart, not
in-flight ask) and [ADR-0009](/pangolin-scale/explanation/decisions/0009-needs-input-sentinel-file-vs-exit-code/) (why sentinel file, not exit code).
...
- [ADR-0008](/pangolin-scale/explanation/decisions/0008-needs-input-request-stop-restart/) — request-stop-restart vs in-flight ask, with the cost
  argument.
- [ADR-0009](/pangolin-scale/explanation/decisions/0009-needs-input-sentinel-file-vs-exit-code/) — why a sentinel file and not an exit code.
- MVP spec §6.9 — the formal protocol.
- [Dispatch lifecycle](/pangolin-scale/reference/dispatch-lifecycle/) — how `dispatch.needs_input`

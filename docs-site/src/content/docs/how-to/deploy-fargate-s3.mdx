...
>
> — [`2026-05-29-pangolin-offload-v1-design.md`](https://github.com/quarrysystems/pangolin-scale/blob/main/docs/superpowers/specs/2026-05-29-pangolin-offload-v1-design.md) (status header)
...
- A working local dispatch first. Do the [hello-world example](https://github.com/quarrysystems/pangolin-scale/tree/main/examples/hello-world) on the local stack before swapping providers — it isolates "my wiring is wrong" from "my AWS setup is wrong."
...
- [Export & verify an audit bundle](/pangolin-scale/how-to/verify-audit-bundle/) — produce and check the tamper-evident bundle this anchor backs.
- [Audit & guarantee tiers](/pangolin-scale/explanation/audit-guarantee-tiers/) — the full model behind `detect` / `external-immutable` / `witnessed` and the `tamper-detecting` vs `tamper-evident` distinction.

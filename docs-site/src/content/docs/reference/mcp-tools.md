---
title: MCP tools
description: The run-time MCP tool surface exposed by agora-mcp — three catalog reads, three dispatch operations, three orchestrator operations.
sidebar:
  order: 2
---
...
appear here — see [The privilege boundary](/agora/explanation/privilege-boundary/).
...
Registration and assignment run through the [CLI](/agora/reference/cli/)
instead. The reasoning behind keeping privileged operations out of the AI loop
is in [The privilege boundary](/agora/explanation/privilege-boundary/).

:::note
The README describes the agora-mcp surface as "exactly six run-time tools."
That count reflects the original surface (three catalog reads + three dispatch
operations). The code exposes **nine**: the three `agora_orchestrator_*` client
tools were added when the offload orchestrator landed. The authoritative list
is the `AGORA_TOOL_NAMES` tuple in `agora-mcp`.
:::

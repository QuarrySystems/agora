...
The `agora-mcp` stdio server exposes the six run-time tools from §4.6 to any
MCP client that can talk to it. The question is what authentication and
authorization model agora-mcp itself should enforce: per-call auth, per-
...
- **Adding per-call auth in MVP** means inventing a token scheme, a
  ...
  Substantial scope.
- **Hosted or multi-tenant agora** absolutely needs more — per-call auth,
...
> **`agora-mcp` authentication is "whoever launched the server."** There is
> no per-call authentication, no per-orchestrator scoping, no granular
> MCP-level ACL. Anyone who can launch the agora-mcp stdio server (typically
...
> host-IAM scoping is sufficient; for hosted or multi-tenant agora, more is
> required.

The host's IAM is the trust boundary. agora-mcp does not authenticate; it
...
- The threat model is explicit: if an attacker can run a process as the
  user/role that launched agora-mcp, they can dispatch arbitrary workers
  against the registry. The mitigations are at the OS / container /
  cloud-IAM layer, not in agora.
- Multi-tenant or hosted agora deployments cannot use MVP agora-mcp as-is.
...
- The deploy-time / run-time split (ADR-0005) still holds: even with full
  ...
  or env bundles through agora-mcp. The blast radius is "dispatch using
...
- Documentation must call this out loudly. Integrators who assume
  ...
  agora-mcp package docs should state the trust-boundary contract in the

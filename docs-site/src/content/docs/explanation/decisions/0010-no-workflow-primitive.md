---
title: "ADR-0010: No Pangolin Scale.workflow() / Pangolin Scale.procedure() primitive"
description: "No Pangolin Scale.workflow() / Pangolin Scale.procedure() primitive in MVP; integrators wrap dispatch() themselves. PARTIALLY SUPERSEDED by ADR-0018 — Pangolin Scale now ships orchestration as a separate layer."
status: superseded
...
The **narrow** decision below still holds: there is no `pangolin.workflow()` /
`pangolin.procedure()` sugar primitive on the client SDK. But this ADR's **broad
posture** — that orchestration "sits above Pangolin Scale, not inside it" and is "out of
...
```ts
const codeReview = Pangolin Scale.workflow({
...
- §11 explicitly lists "Workflow primitives (fan-out, branching)" as out of scope forever — not just deferred — because the architectural posture is that orchestration sits *above* Pangolin Scale, not *inside* it.
...
The decision turns on whether Pangolin Scale ships the "named pre-composed dispatch" abstraction itself, or treats it as integrator-side ergonomics that emerge from observation rather than fiat.
...
```ts
function codeReview(client: PangolinClient, pr: number) {
...
Revisit only if multiple independent integrators converge on the same wrapper shape — that is the signal that an abstraction has actually emerged in the wild and that Pangolin Scale is justified in canonicalizing it.
...
- Multi-step pipelines (fan-out, sequencing, conditional dispatch) are built with ordinary control flow in integrator code, possibly leveraging their existing job/queue infrastructure. Pangolin Scale doesn't try to be a workflow runner.
...
- Ergonomic appeal is paid out by integrators (writing the wrapper) rather than by Pangolin Scale (designing and maintaining the abstraction). For the MVP audience, this is the correct allocation: integrators who can use Pangolin Scale at all can write a five-line wrapper.

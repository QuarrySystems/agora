...
Quarry Systems hosts several libraries today, with an existing Nx-managed monorepo for the platform's primary applications. A future `quarry-systems-platform` reference-orchestrator repo is also on the roadmap. Three plausible homes for agora existed:
...
2. **Inside a future `quarry-systems-platform` repo**, as a sibling of the reference orchestrator that consumes it. Tight feedback loop between agora and its largest internal consumer.
...
Agora lives in its own repository. Per §10.1 of `docs/superpowers/specs/2026-05-21-agora-mvp-design.md`:

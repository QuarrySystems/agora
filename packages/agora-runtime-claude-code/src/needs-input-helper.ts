// Authors the `agora-needs-input-helper` SKILL.md content per §6.9 + §6.6 and
...
// spawn — suppressible via `AGORA_DISABLE_NEEDS_INPUT_HELPER=true`.

import { readFile } from 'node:fs/promises';
import { join } from 'node:path';

const SKILL_OVERLAY_PATH = '.claude/skills/agora-needs-input/SKILL.md';
...
export function isHelperDisabled(env: Record<string, string>): boolean {
  return env.AGORA_DISABLE_NEEDS_INPUT_HELPER === 'true';
}

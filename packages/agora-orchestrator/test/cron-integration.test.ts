...
// SqliteScheduleStore + serve loop + AgoraOrchestrator wired together. Complements the
...
const mkOrch = () => new AgoraOrchestrator({ store: new SqliteRunStateStore(), executors: { x: immediateExecutor() }, triggers: { manual: new ManualTrigger() }, queues: { default: { concurrency: 4 } } });

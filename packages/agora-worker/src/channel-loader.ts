// agora-worker: channel-loader (§6.8).
//
// Reads `agora-channel.json` from the post-overlay workspace, resolves the
...
// `<workspaceDir>/.agora/channel/inbox.jsonl`.
//
...
export async function loadChannelIfPresent(
  ...
  const manifestPath = join(opts.workspaceDir, "agora-channel.json");
  try {
  ...
  const adapter = await loadChannelAdapter(
    cfg.adapter,
    opts.adaptersRoot ?? "/opt/agora/adapters",
  );

  const channelDir = join(opts.workspaceDir, ".agora", "channel");
  ...
  const loop = (async () => {
    while (!stopped) {
      ...
      try {
        ...
        console.error(
          `agora-worker: channel adapter ${cfg.adapter} errored: ${String(err)}`,
        );
        return;
      }
      if (next.done) return;
      ...
      try {
        ...
        // eslint-disable-next-line no-console
        console.error(
          `agora-worker: failed to append channel message to ${inboxPath}: ${String(err)}`,
        );
    ...
    // eslint-disable-next-line no-console
    console.error(
      `agora-worker: channel loop crashed for ${cfg.adapter}: ${String(err)}`,
    );
  });
...
async function loadChannelAdapter(
  ...
  try {
    ...
    throw new Error(
      `agora-worker: channel adapter ${name} not found at ${adapterDir}`,
    );
  }
  ...
  if (typeof factory !== "function") {
    throw new Error(
      `agora-worker: channel adapter ${name} at ${entryPath} does not export a default factory`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `agora-worker: channel adapter ${name} did not return a valid ChannelAdapter (missing name or subscribe())`,
    );
  }
  return adapter;
}

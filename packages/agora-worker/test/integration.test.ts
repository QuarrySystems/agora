...
// imports `@quarry-systems/agora-runtime-claude-code`. That property is the
...
// `@quarry-systems/agora-worker` package alias because pnpm + vitest does
...
/**
 * Mirror of `agora-client.serializeCapabilityBundle`: header line of
...
// `@quarry-systems/agora-runtime-claude-code` — that is the seam contract.
// ---------------------------------------------------------------------------
...
export default () => ({
  ...
  invoke: async (spec, ctx) => {
    ...
    if (scenario === 'needs-input-valid') {
      const dir = join(spec.workspaceDir, '.agora');
...
async function setupHarness(): Promise<Harness> {
  const storageRoot = await mkdtemp(join(tmpdir(), 'agora-int-storage-'));
  const adaptersRoot = await mkdtemp(join(tmpdir(), 'agora-int-adapters-'));
  const workspaceDir = await mkdtemp(join(tmpdir(), 'agora-int-work-'));
 ...
 * (e.g. `MOCK_ADAPTER_SCENARIO`). `AGORA_STORAGE_URI` is set to a `file://`
...
function buildEnv(
  ...
  return {
    AGORA_DISPATCH_ID: 'd-integration',
    AGORA_NAMESPACE: 'ns',
    AGORA_STORAGE_URI: `file://${h.storageRoot}`,
    AGORA_BUNDLE_REFS_JSON: JSON.stringify(refs),
...
function buildDeps(h: Harness, extra: Partial<RunWorkerDeps> = {}): RunWorkerDeps {
  return {
    ...
    secretsManagerClient: {
      // Unused in these tests (no AGORA_CALLBACK_URL configured), but the
...
describe('worker lifecycle (RuntimeAdapter seam smoke test)', () => {
  ...
  itPosix(
    'fails with reason: worker-failed when agora-setup.sh exceeds the configured timeout',
    async () => {
      const h = harness();
      // Stage a capability bundle that overlays an agora-setup.sh that
      ...
      const { contentHash: slowCapHash } = await h.storage.put(
        'agora://ns/capability/slow-cap',
        slowCapBytes,
      );
      const slowCapUri = `agora://ns/capability/slow-cap/${slowCapHash}`;

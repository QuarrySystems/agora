...
async function writeManifest(payload: Record<string, unknown>): Promise<void> {
  await writeFile(
    join(workDir, "agora-channel.json"),
...
describe("loadChannelIfPresent", () => {
  it("returns null when agora-channel.json is absent", async () => {
  ...
  it("loads the named adapter and writes received messages to inbox.jsonl", async () => {
    ...
    const inboxPath = join(workDir, ".agora", "channel", "inbox.jsonl");
  ...
  it("passes the configured channel and opts to the adapter", async () => {
    ...
    const text = await readFile(
      join(workDir, ".agora", "channel", "inbox.jsonl"),
      "utf-8",
    );
  ...
  it("creates the .agora/channel directory if missing", async () => {

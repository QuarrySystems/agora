...
describe('LifecycleEmitter', () => {
  ...
  describe('emit', () => {
    ...
    it('POSTs to callbackUrl with HMAC signature headers', async () => {
      ...
      expect(opts.headers['X-Agora-Dispatch-Id']).toBe(dispatchId);
      expect(opts.headers['X-Agora-Timestamp']).toBeDefined();
      expect(opts.headers['X-Agora-Signature']).toBeDefined();

      const payload = opts.body;
      const signature = opts.headers['X-Agora-Signature'];
      const timestamp = opts.headers['X-Agora-Timestamp'];

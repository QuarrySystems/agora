...
import type { ProviderContext, TaskSpec } from '@quarry-systems/agora-core';
...
afterAll(async () => {
  ...
  try {
    const containers = await docker.listContainers({
      all: true,
      filters: { label: ['agora.test=local-docker-integration'] },
    });
...
describe('LocalDockerProvider — integration against real Docker daemon', () => {
  ...
  itIf(dockerAvailable)(
    'concurrent dispatches carry disjoint agora.dispatchId labels',
    async () => {
      const provider = new LocalDockerProvider({ docker });
      ...
      try {
        ...
        expect(infoA.Config.Labels?.['agora.dispatchId']).toBe(dispatchA);
        expect(infoB.Config.Labels?.['agora.dispatchId']).toBe(dispatchB);

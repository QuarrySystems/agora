...
const FORBIDDEN_PREFIXES = ['@quarry-systems/agora-core'];

describe('@quarry-systems/agora-providers-local-docker scaffold shape', () => {
  describe('package.json', () => {
    let pkg: Record<string, unknown>;

    it('(1) name is @quarry-systems/agora-providers-local-docker', () => {
      pkg = readJson('package.json') as Record<string, unknown>;
      expect(pkg.name).toBe('@quarry-systems/agora-providers-local-docker');
    });

    it('(2) dependencies includes @quarry-systems/agora-core at workspace:*', () => {
      pkg = readJson('package.json') as Record<string, unknown>;
      const deps = pkg.dependencies as Record<string, string> | undefined;
      expect(deps).toBeDefined();
      expect(deps?.['@quarry-systems/agora-core']).toBe('workspace:*');
    });
    ...
    it('(3) no forbidden external @quarry-systems/* prefixes in devDependencies or peerDependencies', () => {
      ...
      const checkDeps = (deps: Record<string, string> | undefined, label: string) => {
        ...
        for (const key of Object.keys(deps)) {
          // agora-core is allowed as a workspace dep in dependencies but
          // no other @quarry-systems/* packages should appear in dev/peer
          if (key.startsWith('@quarry-systems/') && key !== '@quarry-systems/agora-core') {

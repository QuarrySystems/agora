...
//
// Smoke tests run against LocalStack when AGORA_TEST_S3_ENDPOINT is set.
...
import { ConflictError } from '@quarry-systems/agora-core';
...
describe('S3StorageProvider index conditional writes', () => {
  it('writes the first index entry with IfNoneMatch: *', async () => {
    ...
    await sp.put('agora://ns/capability/foo', new TextEncoder().encode('x'));
  ...
  it('writes a subsequent index entry with IfMatch: <etag>', async () => {
    ...
    await sp.put('agora://ns/capability/foo', new TextEncoder().encode('a'));
    await sp.put('agora://ns/capability/foo', new TextEncoder().encode('b'));
  ...
  it('throws ConflictError after exhausting retries', async () => {
    ...
    await expect(
      sp.put('agora://ns/capability/foo', new TextEncoder().encode('b')),
  ...
  it('no-ops the index update when the contentHash is already present', async () => {
    ...
    await sp.put('agora://ns/capability/foo', payload);
    fake.commands.length = 0;

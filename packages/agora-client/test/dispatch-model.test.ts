...
describe('client.dispatch model passthrough', () => {
  ...
  it('emits AGORA_MODEL verbatim when work.model is set', async () => {
    ...
    expect(spec!.env.AGORA_MODEL).toBe('max'); // verbatim — no level mapping client-side
  });

  it('does not emit AGORA_MODEL when work.model is unset', async () => {
    const { client, getLastSpec } = makeHarness();
    ...
    const spec = getLastSpec();
    expect(spec).toBeDefined();
    expect('AGORA_MODEL' in spec!.env).toBe(false);
  });
  ...
  it('passes through model level values verbatim (fast, standard, max)', async () => {
    ...
    for (const level of levels) {
      ...
      const spec = getLastSpec();
      expect(spec!.env.AGORA_MODEL).toBe(level);
    }
  });
  ...
  it('passes through a provider-native model id verbatim (no mapping)', async () => {
    ...
    const spec = getLastSpec();
    expect(spec!.env.AGORA_MODEL).toBe('claude-opus-4-5');
  });

  it('does not apply client.defaultModel to AGORA_MODEL when work.model is unset (no client-side def-fallback)', async () => {
    ...
    // Client is constructed WITH a defaultModel — it must NOT be injected as AGORA_MODEL
    ...
    // Key must be ABSENT — client must NOT fall back to defaultModel for AGORA_MODEL
    expect('AGORA_MODEL' in capturedSpec!.env).toBe(false);
  });

  it('emits empty-string AGORA_MODEL is excluded (treats empty string as unset)', async () => {
    // Per the implementation contract: `work.model !== ''` guard ensures empty
    // string is treated the same as undefined — AGORA_MODEL is not emitted.
    const { client, getLastSpec } = makeHarness();

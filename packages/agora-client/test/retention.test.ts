...
import type { DispatchResult, StorageProvider } from '@quarry-systems/agora-core';
...
function makeClient(storage: StorageProvider, maxDays = 30): AgoraClient {
...
describe('writeDispatchRecord', () => {
  ...
  it('writes the record under agora://<ns>/dispatches/<dispatchId>/record.json', async () => {
    const storage = makeMemoryStorage();
    const client = makeClient(storage, 30);
    ...
    const expectedUri = 'agora://o/dispatches/d1/record.json';
  ...
  it('serializes the record including providerTaskId, target, retentionDays, recordedAt', async () => {
    ...
    const bytes = storage.blobs.get('agora://o/dispatches/d1/record.json')!;

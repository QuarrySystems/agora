// Integration tests for `@quarry-systems/agora-client` against the real
...
// integration test. `agora-storage-local/test/integration.test.ts` covers
...
// `client.dispatch`) onto `AgoraClient.prototype`. Without this import the
...
beforeEach(async () => {
  storageRoot = await mkdtemp(join(tmpdir(), 'agora-client-int-'));
});
...
describe('agora-client integration — register side (no Docker required)', () => {
  ...
  it(
    ...
    async () => {
      ...
      const versions = await client.storage.list(
        `agora://tests/subagent/sub-evolve`,
      );
      ...
      try {
        ...
        try {
          const containers = await docker.listContainers({
            all: true,
            filters: { label: [`agora.dispatchId=${result.dispatchId}`] },
          });

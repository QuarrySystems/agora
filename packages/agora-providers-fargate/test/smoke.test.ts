...
const PINNED = 'public.ecr.aws/agora/worker@sha256:' + 'a'.repeat(64);
...
describe('FargateProvider.run — image pin enforcement (§7.4)', () => {
  it('rejects images without a digest pin', async () => {
    const provider = new FargateProvider(baseOpts({ ecsClient: fakeEcs({}) }));
    await expect(
      provider.run(baseSpec({ image: 'agora-worker:latest' }), baseCtx),
    ).rejects.toBeInstanceOf(UnpinnedImageError);
  });

  it('rejects bare image names without a digest pin', async () => {
    const provider = new FargateProvider(baseOpts({ ecsClient: fakeEcs({}) }));
    await expect(provider.run(baseSpec({ image: 'agora-worker' }), baseCtx)).rejects.toBeInstanceOf(
      UnpinnedImageError,
    );
  });
  ...
  it('rejects sha256 references not in the @sha256: form', async () => {
    const provider = new FargateProvider(baseOpts({ ecsClient: fakeEcs({}) }));
    await expect(
      provider.run(baseSpec({ image: 'agora-worker:sha256-' + 'a'.repeat(64) }), baseCtx),
    ).rejects.toBeInstanceOf(UnpinnedImageError);
  });
  ...
  it('allows an unpinned image when allowUnpinnedImage: true', async () => {
    ...
    const handle = await provider.run(baseSpec({ image: 'agora-worker:latest' }), baseCtx);
...
describe('FargateProvider.run — RunTask contract', () => {
  it('calls RunTaskCommand with cluster, task-definition family, launchType FARGATE, and awsvpc network config', async () => {
    ...
    const provider = new FargateProvider(
      baseOpts({
        ...
        taskDefinitionFamily: 'agora-worker-prod',
    ...
    expect(input.taskDefinition).toBe('agora-worker-prod');
  ...
  it('passes env, command and resources as container overrides', async () => {
    ...
    expect(co.name).toBe('agora-worker');
  ...
  it('tags the task with agora:dispatchId for post-hoc discovery', async () => {
    let captured: RunTaskCommand | null = null;
    const provider = new FargateProvider(
      baseOpts({
        ecsClient: fakeEcs({
    ...
    expect(captured!.input.tags).toEqual(
      expect.arrayContaining([{ key: 'agora:dispatchId', value: 'dispatch-xyz' }]),
    );
  });
    ...
    try {
      const states = [
        ...
        { lastStatus: 'RUNNING' },
        {
          ...
          containers: [{ name: 'agora-worker', exitCode: 0 }],
        },
      ];
      let i = 0;
      const ecs = fakeEcs({
          ...
          tasks: [
            {
              ...
              containers: [{ name: 'agora-worker', exitCode: 137 }],
            },
          ],
        }),
      });
...
describe('FargateProvider.cancel — StopTask', () => {
  it('calls StopTaskCommand with the configured cluster, task ARN, and reason "agora.cancel"', async () => {
    ...
    expect(captured!.input.reason).toBe('agora.cancel');
  });
});

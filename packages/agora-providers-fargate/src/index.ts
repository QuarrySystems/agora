...
// agora-cancel reason marker; ECS handles the SIGTERM/SIGKILL grace dance
  ...
  async run(spec: TaskSpec, _ctx: ProviderContext): Promise<TaskHandle> {
    ...
    const res = await this.ecs.send(
      new RunTaskCommand({
        ...
        tags: [{ key: 'agora:dispatchId', value: spec.dispatchId }],
      }),
    );
